...
describe('TermsOfServicePage', () => {
  ...
  it('displays service description', () => {
    ...
    expect(screen.getByText(/本利用規約（以下「本規約」）は、jt-chihara（以下「当方」）が提供する割り勘アプリ「warikan」/)).toBeInTheDocument();
  });
  ...
  it('shows prohibited actions list', () => {
    ...
    expect(screen.getByText('他のユーザーに関する個人情報等を収集または蓄積する行為')).toBeInTheDocument();
  });

  it('displays service suspension conditions', () => {
    render(<TermsOfServicePage />);
    
    expect(screen.getByText('本サービスにかかるコンピュータシステムの保守点検または更新を行う場合')).toBeInTheDocument();
    expect(screen.getByText(/地震、落雷、火災、停電または天災などの不可抗力により、本サービスの提供が困難となった場合/)).toBeInTheDocument();
    expect(screen.getByText('コンピュータまたは通信回線等が事故により停止した場合')).toBeInTheDocument();
  });
  ...
  it('displays copyright and disclaimer sections', () => {
    ...
    // 著作権関連の文言
    expect(screen.getByText(/ユーザーは、自ら著作権等の必要な知的財産権を有するか/)).toBeInTheDocument();
  ...
  it('shows jurisdiction information', () => {
    ...
    expect(screen.getByText(/本規約の解釈にあたっては、日本法を準拠法とします/)).toBeInTheDocument();
  });
});